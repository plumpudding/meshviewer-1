    ...
    return function (config) {
      function handleData(data) {
        ...
        for (var i = 0; i < data.length; ++i) {
          ...
          if (i % 2) {
            ...
            if (data[i].version !== 1) {
              ...
              if (data[i].timestamp) {
                ...
                dataGraph.timestamp = data[i]._lastModify;
              }
            }
          /**
           * nodes.json block
           ...
           */
          } else if (data[i].version !== 2 && data[i].version !== undefined) {
            vererr = 'Unsupported nodes version: ' + data[i].version;
            console.error(vererr); // silent fail
          } else {
            var transformDataNodes = data[i].nodes;
            if (data[i].version === undefined) {
              transformDataNodes = helper.mapToArray(data[i].nodes);
            }
